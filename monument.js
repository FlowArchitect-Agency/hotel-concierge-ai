...
function start(){
  ...
  // The headline slot: the chapter line leads while the stone assembles, then
  // one fact per stretch of the turn, each held long enough to read. They are
  ...
  function showCard(p){
    const n = CARDS.length - 1;                       // cards after the chapter line
    if (n < 1) return;
    const i = p < FACTS_FROM ? 0
      : Math.min(n, 1 + Math.floor((p - FACTS_FROM) / ((1 - FACTS_FROM) / n)));
    if (i === shownCard) return;

    const leaving = CARDS[shownCard];
    // outgoing cards exit the way the reader is travelling
    leaving.classList.remove('is-on');
    leaving.classList.add(i > shownCard ? 'was-above' : 'was-below');
  ...
  function onScroll(){
    ...
    stage.classList.toggle('is-assembled', p > 0.055);
    showCard(p);
  }
